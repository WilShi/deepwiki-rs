#[cfg(test)]
mod tests {
    use crate::config::{Config, LLMProvider, LLMConfig, CacheConfig};
    ...
    fn test_llm_provider_from_str() {
        ...
        assert_eq!("moonshot".parse::<LLMProvider>().unwrap(), LLMProvider::Moonshot);
        assert_eq!("deepseek".parse::<LLMProvider>().unwrap(), LLMProvider::DeepSeek);
        assert_eq!("mistral".parse::<LLMProvider>().unwrap(), LLMProvider::Mistral);
...
"#;
        ...
        assert_eq!(config.extract_from_cargo_toml(), Some("test-crate".to_string()));
    }
...
"#;
        ...
        assert_eq!(config.extract_from_package_json(), Some("test-package".to_string()));
    }
...
"#;
        ...
        assert_eq!(config.extract_from_pyproject_toml(), Some("test-project".to_string()));
    }
...
"#;
        ...
        assert_eq!(config.extract_from_pyproject_toml(), Some("poetry-project".to_string()));
    }
...
"#;
        ...
        assert_eq!(config.extract_from_pom_xml(), Some("Test Project".to_string()));
    }
