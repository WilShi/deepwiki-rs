...
为了帮助开发者快速理解和使用代码，**每个重要的函数和类都必须包含实用信息**：

#### 对于函数/方法：
```markdown
#### `function_name(param1: Type1, param2: Type2) -> Result<ReturnType>`
📁 **定义位置**: `src/module/file.rs:123`

**使用示例**：
```rust
// 基本调用
let result = module.function_name(value1, value2)?;
```
...
**错误处理**：
- `ErrorType1` → 错误原因和解决方法
- `ErrorType2` → 错误原因和解决方法

**相关测试**：`tests/module_test.rs:45`
```

#### 对于数据结构：
```markdown
### StructName
📁 **定义位置**: `src/models/file.rs:20`
