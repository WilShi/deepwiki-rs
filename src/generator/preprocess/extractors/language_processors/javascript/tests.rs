...
"#;
    ...
    fn test_determine_component_type() {
        ...
        // Test main files
        assert_eq!(
            processor.determine_component_type(Path::new("index.js"), ""),
            "js_main"
        );

        assert_eq!(
            processor.determine_component_type(Path::new("main.js"), ""),
            "js_main"
        );

        // Test config files
        assert_eq!(
            processor.determine_component_type(Path::new("app.config.js"), ""),
            "js_config"
        );

        // Test test files
        assert_eq!(
            processor.determine_component_type(Path::new("app.test.js"), ""),
            "js_test"
        );
        ...
        // Test plain file
        assert_eq!(
            processor.determine_component_type(Path::new("data.txt"), ""),
            "js_file"
        );
    }
...
"#;
        ...
        if let Some(_user_service) = interfaces
            .iter()
            .find(|i| i.name == "UserService" && i.interface_type == "class")
        {
            // Found the class
        }
